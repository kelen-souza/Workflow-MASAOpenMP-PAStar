...
@task(work_dir=IN, seq1=IN, seq2=IN, alignment_file=IN, seq_dir=IN)
def masa_wrapper(work_dir, threads, seq1, seq2, alignment_file, seq_dir):
     import os
     seq_1_path = os.path.join(seq_dir, seq1)
     seq_2_path = os.path.join(seq_dir, seq2)
     print(seq_1_path)
     print(seq_2_path)
     r_masa = masa(work_dir, threads, seq_1_path, seq_2_path, alignment_file)
     return r_masa

@constraint(computing_units="1")
@binary(binary=MASA_BINARY, args="--work-dir {{work_dir}} -t {{threads}} {{input_1}} {{input_2}}")
@task(work_dir=IN, input_1=IN, input_2=IN, alignment_file=IN)
...
@task(pair_dir=INOUT, alignment_file = IN, returns=dict)
def get_metrics(pair_dir, alignment_file):
     ...
     """
     import re, os
     align_out = os.path.join(pair_dir, alignment_file)
